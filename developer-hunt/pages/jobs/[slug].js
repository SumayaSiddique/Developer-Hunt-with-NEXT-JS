...
const ProblemDetails = ({ data, userDetails }) => {
    ...
    return (
        <div>
            <Head>
                <title>Job Details | Developer Hunt</title>
                <meta name="viewport" content="initial-scale=1.0, width=device-width" />
            </Head>
            <div className="my-10 flex flex-col items-center">
                <div className="max-w-lg">
                    <p className="text-3xl  my-5 font-medium">{data.Job_name}</p>
                    <p className="text-lg  my-5 opacity-40 font-semibold ">Company: {data.Company}</p>
                    <p className="text-lg  my-5 opacity-40 ">Type: {data.Type}</p>
                    <p className="text-xl  my-5 font-semibold">Salary : {data.Salary} </p>
                    <hr />
...
export async function getStaticProps({ params }) {
    ...
    const result = await Axios.get('http://localhost:3001/job/id', { params: { Job_Id: params.slug } })
...
export const getStaticPaths = async () => {
    ...
    const paths = items.map(item => {
        return {
            params: { slug: item.Job_Id.toString() }
        }
    })
